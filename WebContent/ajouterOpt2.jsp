...
<body
	...
	<form role="form" action="AddOptionServlet">
	<input type="hidden" name="nbOpt" value="<%=request.getParameter("nbOpt")%>" />
	<input type="hidden" name="idBallot" value="<% out.print(request.getParameter("idBallot"));%>" />
			<% int n = Integer.parseInt(request.getParameter("nbOpt")); 
				for(int i = 1; i<=n;i++){
			%>
		<div class="form-group">
			<!-- Username -->
			<label class="col-md-4 control-label t" for="username">Option
				<%=i%> : </label>
			<div class="col-md-4 controls">
				<input type="text" id="opt<%=i%>" name="opt<%=i%>" placeholder=""
					class="form-control">
			</div>
		</div>
			<% } %>
