...
<body
	...
	<%@ page import="Model.*,Classes.*,java.util.*" %>
	
	<% 
	 	TBallotEntity b = new TBallotModel().getBallotById(Integer.parseInt(request.getParameter("idBallot")));
		List<TOptionEntity> l = new TOptionModel().getOptionsByBallot(b.getIdBallot());				
		int i = 1;				
	%>
	<form role="form" action="VoteServlet">
	<input type="hidden" name="idBallot" value=<%=b.getIdBallot() %> />
		<div class="form-group">
			<!-- Username -->
			<label class="col-md-4 control-label t" for="username">Titre
				Vote</label>
			<div class="col-md-4 controls">
				<input type="text" id="nomB" name="nomB" value="<%=b.getLibBallot()%>" placeholder=""
		...
		<div class="col-md-4 btn-group" data-toggle="buttons">
			<!-- E-mail -->
			<% for(TOptionEntity o : l){ %>
			<label class="col-md-4 radio"><input type="radio" name="conf"
				value="<%=o.getIdOption()%>"><%=o.getLibOption() %></label> 
			<% 
				i++;
				} 
			%>
